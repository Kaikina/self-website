  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a:
          "Oui — PrestaShop, c'est là où passe la majorité de mon temps, et c'est la stack que je mettrais en avant en premier si tu cherches à savoir si je suis le bon profil. Je travaille au niveau du core et des modules en 1.7, en 8 et sur la version 9 actuelle : modules sur mesure, setups multishop, intégrations ERP et systèmes tiers, travail de thème, et les passes de performance et de sécurité qu'une boutique accumule en grandissant. Je contribue aussi au core PrestaShop lui-même sous le pseudo @Kaikina sur GitHub, surtout en corrigeant les frictions que je rencontre en mission plutôt qu'en chassant des tickets de feature. Cette double vue — construire au-dessus de PrestaShop toute la journée et patcher de temps en temps la brique en dessous — c'est ce qui fait souvent la différence sur les problèmes tordus. La majorité des projets listés sur ce site sont des builds PrestaShop : airsoft, chaussures, textile, collection, phytothérapie, pièces CVC. Si ton projet est sous PrestaShop, c'est exactement mon métier.",
      },
      {
        q: "Peux-tu prendre en charge une migration PrestaShop (1.5 → 1.7 → 8 → 9) ?",
        a:
          "Oui — c'est une des demandes qui reviennent le plus souvent. J'ai livré des migrations PrestaShop multi-étapes sur plusieurs projets concrets : de grandes migrations multishop, des montées de version majeure qui ont nécessité un travail poussé au niveau du core, et des phases où un install legacy devait rester en production pendant qu'on construisait son remplaçant à côté. La même approche se prolonge sur PrestaShop 9, la version majeure actuelle. Le saut de version lui-même est rarement la partie dure. Ce qui mange le temps, c'est tout ce qui pend à la boutique — les modules tiers et sur mesure qui supposent les anciennes API, une synchro ERP qui doit continuer à faire correspondre commandes et stock tout du long, des overrides de thème écrits contre des internals qui ont bougé, et les années de règles métier enfouies dans la base que personne n'a documentées. Du coup je planifie une migration autour de ces risques d'abord et je traite la montée du core comme la partie qui se passe comme prévu, parce que c'est en général le cas, et eux non.",
      },
      {
        q: "Et les intégrations ERP ?",
        a:
          "La plupart des projets PrestaShop sur lesquels je travaille finissent par toucher un ERP ou un système tiers, donc l'intégration est une partie centrale du job, pas une quête annexe. J'ai connecté des boutiques à des ERP, des flux de données de marché et des outils de support, et en pratique l'intégration prend une de ces trois formes : appels webservice synchrones quand la réponse doit être en temps réel, batchs planifiés en cron quand une réconciliation nocturne suffit, et pipelines asynchrones en queue quand le volume est élevé ou que le tiers est lent et qu'on ne peut pas laisser un client attendre dessus. Choisir la mauvaise forme, c'est là que ces projets font mal — un appel synchrone qui lie le checkout à un ERP instable, ou un batch qui dérive discrètement entre deux exécutions. Donc le vrai travail n'est pas d'écrire le connecteur ; c'est d'accorder le pattern au volume de données, aux SLA et à la staleness que le métier peut réellement tolérer. J'ai livré les trois.",
      },
      {
